    ...
    <div className={css.todayContainer}>
      ...
      {waterPortions?.length > 0 && (
        <ul className={css.listWaters}>
          {waterPortions
            .slice()
            .sort((a, b) => getConvertedTime(a.time).getTime() - getConvertedTime(b.time).getTime())
            .map((item) => (
      ...
      {isDelete ? (
        <ModalContainer isOpen = {isModalOpen} onClose={closeModal} selectedItemId = {selectedItem?.id} >
          <div className={css.containerDel}>
            <div className={css.firstblock}>
              <h2 className={css.title}>Delete entry</h2>
              <button className={css.exit} type="button" onClick={closeModal} aria-label="Close">
            ...
            <div className={css.containerBtnDel}>
              <button className={css.btnCancellDel} type="button" onClick={closeModal}>
                Cancel
              </button>
